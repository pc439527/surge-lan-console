...
export function AutomationPage() {
  ...
  return <div className="space-y-5 lg:space-y-6"><PageHeader eyebrow="Automation" title="后台任务" description="Scheduler 与 Collector 运行在 Local Core；关闭浏览器后仍持续执行。" actions={<Button variant="secondary" size="sm" onClick={() => void refresh()}><RefreshCw className="h-4 w-4" />刷新</Button>} />
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
      <Card><CardHeader><CardTitle>Scheduled Jobs</CardTitle><p className="text-xs text-text-tertiary">默认：Metrics 60s、Events 30s、DNS 10min、Node Quality 30min、配置快照 6h。Profile Reload 与 Daily Digest 默认关闭。</p></CardHeader><CardContent className="space-y-2">{jobs.isLoading ? <p className="py-5 text-sm text-text-secondary">加载任务…</p> : jobs.data?.length ? jobs.data.map((job) => <JobRow key={job.id} job={job} connectionName={job.connectionId ? connections.find((c) => c.id === job.connectionId)?.name ?? job.connectionId : "全局"} onChanged={refresh} />) : <p className="py-5 text-center text-sm text-text-secondary">暂无后台任务。</p>}</CardContent></Card>
...
function jobLabel(type: string): string {
  const labels: Record<string, string> = { "device-heartbeat": "Device Heartbeat", metrics: "Metrics Collector", events: "Event Collector", "dns-health": "DNS Health Check", "node-health": "Node Quality Check", "profile-snapshot": "配置快照", "profile-reload": "Profile Reload", "daily-digest": "Daily Digest" };
