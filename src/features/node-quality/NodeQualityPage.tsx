...
export function NodeQualityPage() {
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-[26px] font-semibold text-text-primary">节点质量中心</h1><p className="mt-0.5 text-sm text-text-secondary">统一查看节点延迟、可用性和综合评分。</p></div><div className="flex gap-2"><Button onClick={() => testAll.mutate((groups.data ?? []).map((group) => ({ name: group.name, policies: group.policies.map((name) => ({ name, typeDescription: group.types[name] ?? "", lineHash: group.lineHashes[name] })) })))} disabled={testAll.isPending || !groups.data?.length}>{testAll.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}{testAll.isPending ? "测速中…" : "一键测速"}</Button><Button variant="secondary" onClick={() => groups.refetch()} disabled={testAll.isPending}><RefreshCw className="h-4 w-4" />刷新节点</Button></div></header>
    ...
    {measured.length === 0 && rows.length > 0 && <Card><CardContent className="flex flex-col items-center gap-2 py-8 text-center"><Gauge className="h-6 w-6 text-accent" /><p className="text-sm font-medium text-text-primary">等待节点测速数据</p><p className="text-xs text-text-secondary">点击右上角“一键测速”，系统会依次测试全部策略组。</p></CardContent></Card>}
