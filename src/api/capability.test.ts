...
describe("detectPlatform", () => {
  it("detects macOS when modules + scripts + rules are all supported", () => {
    expect(detectPlatform(featuresOf({ modules: "supported", scripts: "supported", rules: "supported" }))).toBe("macos");
  });

  it("detects tvOS when the advanced trio is unsupported but core is up", () => {
    expect(
      detectPlatform(featuresOf({ modules: "unsupported", scripts: "unsupported", rules: "unsupported", traffic: "supported" })),
    ).toBe("tvos");
  });

  it("detects iOS when rules work but modules do not", () => {
    expect(detectPlatform(featuresOf({ rules: "supported", modules: "unsupported", traffic: "supported" }))).toBe("ios");
  });

  it("returns unknown when the probe is inconclusive", () => {
...
describe("probeCapabilities", () => {
  function stubClient(overrides: Record<string, unknown>) {
    const probeEndpoint = vi.fn(async (endpoint: string) => {
      ...
      // errProbe/okProbe 已经返回 probe 形状（{status, latencyMs, raw, error}）。
  ...
  it("flags the advanced trio as unsupported on a tvOS-like device", async () => {
    const payloads = {
      ...fullPayloads,
  ...
  it("keeps the whole report usable when an endpoint is unreachable", async () => {
    ...
    expect(report.platform).toBe("macos"); // 核心 trio 仍判定
  });
